...
async def demo_training():
    ...
    try:
        ...
        print("Importing and extracting knowledge...")
        content = await scraper_service.extract_text_from_url(url)
        
        if content:
            print(f"SUCCESS: Extracted {len(content)} characters of conversation.")
            print("Training Master Agent...")
            num_patterns = await master_agent.extract_knowledge_from_logs(content)
            print(f"FINISH: Extracted {num_patterns} patterns into permanent memory.")
        ...
        print(f"ERROR: Demo runtime error: {e}")
